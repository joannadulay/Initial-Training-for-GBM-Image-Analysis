...
MODEL_PATH = MODEL_DIR / "svm_model_cv.pkl"
...
def main():
    ...
    # SVMs are distance-based, unlike GBM/RF, so features need to be
    # standardized first (this 571-dim vector mixes raw pixel-count ratios,
    # 0-255 texture stats, and normalized histograms on very different
    # scales - without scaling, the large-range features would dominate
    # the decision boundary regardless of how informative each feature
    # actually is). Wrapping the scaler + classifier in one Pipeline keeps
    # the scaler's fit confined to each CV fold's training portion, so no
    # information leaks from validation/test data into the fitted scale.
    pipe = Pipeline([
    ...
    # NOTE: sample_weight for a Pipeline must be passed as step__param
    search.fit(X_dev, y_dev, svm__sample_weight=sample_weight_dev)
    ...
    save_text(OUTPUT_DIR / "svm_final_test_report.txt", "\n".join(report_lines))
    ...
    save_text(OUTPUT_DIR / "svm_final_test_misclassified.txt", "\n".join(mis_lines))
