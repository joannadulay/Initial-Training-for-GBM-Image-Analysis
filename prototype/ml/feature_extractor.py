...
def lesion_shape_features(brown_mask, mask_pixels):
    ...
    NOTE: this is reused as-is on two different masks - the brown/lesion
    mask (original use) and the dark/necrotic mask (added for the
    black-spot shape block in engineered_features).
    """
...
def engineered_features_named(image_bgr, hsv, gray, mask, leaf_ratio, seg_rejected):
    """
    Same computation as engineered_features(), but returned as a named dict
    instead of a flat array - lets callers (like predict_core.py) surface
    individual signals such as yellow_ratio / lesion_count / mean_circularity
    for display, without touching the trained model's feature vector.

    Updated to include the black-spot block (black_on_yellow, has_black_spot,
    and dark-mask shape stats) added during the latest retrain, so the
    "Visual Findings" popup can surface those too if wired up later.
    """
    h, s, v = cv2.split(hsv)
    ...
    lesion_count, mean_area_ratio, std_area_ratio, mean_circularity, fragmentation = \
        lesion_shape_features(brown_mask, mask_pixels).tolist()

    # --- added during retrain: black-spot block ---
    black_on_yellow = dark_ratio * yellow_ratio

    (bs_lesion_count, bs_mean_area_ratio, bs_std_area_ratio,
     bs_mean_circularity, bs_fragmentation) = lesion_shape_features(dark_mask, mask_pixels).tolist()
    has_black_spot = 1.0 if bs_lesion_count > 0 else 0.0
    # --- end added block ---
    ...
    return {
        ...
        "seg_rejected": bool(seg_rejected),
        "black_on_yellow": float(black_on_yellow),
        "has_black_spot": float(has_black_spot),
        "lesion_count": float(lesion_count),
        "mean_area_ratio": float(mean_area_ratio),
        "std_area_ratio": float(std_area_ratio),
        "mean_circularity": float(mean_circularity),
        "fragmentation": float(fragmentation),
        "bs_lesion_count": float(bs_lesion_count),
        "bs_mean_area_ratio": float(bs_mean_area_ratio),
        "bs_std_area_ratio": float(bs_std_area_ratio),
        "bs_mean_circularity": float(bs_mean_circularity),
        "bs_fragmentation": float(bs_fragmentation),
    }


def engineered_features(image_bgr, hsv, gray, mask, leaf_ratio, seg_rejected):
    """
    Flat vector for the model. Order matches the training-side
    feature_extractor.py exactly:
      base_features (14) -> lesion_shape on brown_mask (5) -> black_spot_shape
      on dark_mask (5) = 24 total, vs. the old 12 + 5 = 17.
    """
    named = engineered_features_named(image_bgr, hsv, gray, mask, leaf_ratio, seg_rejected)
    ...
    base_features = np.array([
        ...
        named["std_gray"],
        named["leaf_ratio"],
        1.0 if named["seg_rejected"] else 0.0,
        named["black_on_yellow"],
        named["has_black_spot"],
    ], dtype=np.float32)

    lesion_shape = np.array([
        named["lesion_count"],
        named["mean_area_ratio"],
        named["std_area_ratio"],
        named["mean_circularity"],
        named["fragmentation"],
    ], dtype=np.float32)

    black_spot_shape = np.array([
        named["bs_lesion_count"],
        named["bs_mean_area_ratio"],
        named["bs_std_area_ratio"],
        named["bs_mean_circularity"],
        named["bs_fragmentation"],
    ], dtype=np.float32)
...
def extract_features_with_details_from_bgr(image_bgr):
    """
    Same feature vector as extract_features_from_bgr(), plus a `details`
    dict of the individual named signals (yellow_ratio, lesion_count,
    mean_circularity, edge_density, etc.) for display purposes - e.g. the
    "Visual Findings" section of the details popup in ui_main.py.
    """
    image = cv2.resize(image_bgr, (IMG_SIZE, IMG_SIZE))
    mask, leaf_ratio, seg_rejected = segment_leaf(image)
    ...
    details = engineered_features_named(image, hsv, gray, mask, leaf_ratio, seg_rejected)
    details["edge_density"] = float(f_edge[0])
